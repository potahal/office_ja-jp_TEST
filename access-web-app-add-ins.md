...
8. **[ソリューション エクスプローラー]** でプロジェクトの Web プロジェクト (**<project_name>Web**) を選択します。[プロパティ] ウィンドウで、**[SSL URL]** のエントリを見つけます。これは `https://localhost:44314/` のように表示されています。この URL を選択し、クリップボードにコピーします。この URL は、この後すぐに必要になります。
...
この時点で、ブレークポイントの設定、JavaScript コードのステップの実行、DOM の探索、コードの変更を行って、Access Web アプリ を対象とする Office アドイン に変更箇所が表示されていることを確認できます。詳細については、「 [F12 開発者用ツール](http://msdn.microsoft.com/ja-jp/library/ie/bg182326%28v=vs.85%29)」を参照してください。

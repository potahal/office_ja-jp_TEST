...
|:-----|:-----|:-----|:-----|
| _Address_|必需|**String**|The address of the target document.|
| _SubAddress_|可选|**Variant**|The location within the target document. The default value is an empty string.|
| _NewWindow_|可选|**Variant**|**True** to display the target location in a new window. The default value is **False**.|
| _AddHistory_|可选|**Variant**|**True** to add the link to the current day's history folder.|
| _ExtraInfo_|可选|**Variant**|A string or a byte array that specifies additional information for HTTP to use to resolve the hyperlink. For example, you can use ExtraInfo to specify the coordinates of an image map, the contents of a form, or a FAT file name. The string is either posted or appended, depending on the value of Method. Use the  **ExtraInfoRequired** property to determine whether extra information is required.|

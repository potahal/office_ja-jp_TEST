...
## Syntax
 ...
 _表达式_. **SetMapping**( ** _XPath_**, ** _PrefixMapping_**, ** _Source_** )
...
|:-----|:-----|:-----|:-----|
| _XPath_|必需|**String**|Specifies an XPath string that represents the XML node to which to map the content control. An invalid XPath string causes a run-time error.|
| _PrefixMapping_|可选|**String**|Specifies the prefix mappings to use when querying the expression provided in the XPath parameter. If omitted, Word uses the set of prefix mappings for the specified custom XML part in the current document.|
| _Source_|可选|**CustomXMLPart**|Specifies the desired custom XML data to which to map the content control. If this parameter is omitted, the XPath is evaluated against all custom XML in the current document, and the mapping is established with the first  **CustomXMLPart** in which the XPath resolves to an XML node.|
 ...
 **注释**  Creating a mapping for a rich-text content control causes a run-time error.

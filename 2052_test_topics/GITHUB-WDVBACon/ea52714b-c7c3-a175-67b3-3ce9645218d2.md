...
## Syntax
 ...
 _表达式_. **AddAsk**( ** _Range_**, ** _Name_**, ** _Prompt_**, ** _DefaultAskText_**, ** _AskOnce_** )
...
|:-----|:-----|:-----|:-----|
| _Range_|必需|**Range object**|The location for the ASK field.|
| _Name_|必需|**String**|The bookmark name that the response or default text is assigned to. Use a REF field with the bookmark name to display the result in a document.|
| _Prompt_|可选|**Variant**|The text that's displayed in the dialog box.|
| _DefaultAskText_|可选|**Variant**|The default response, which appears in the text box when the dialog box is displayed. Corresponds to the \d switch for an ASK field.|
| _AskOnce_|可选|**Variant**|**True** to display the dialog box only once instead of each time a new record is merged. Corresponds to the \o switch for an ASK field.|

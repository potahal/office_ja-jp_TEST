...
Private Sub  _表达式_ _ **ContentControlOnExit**( ** _ContentControl_**, ** _Cancel_** )
...
|:-----|:-----|:-----|:-----|
| _ContentControl_|必需|**ContentControl**|The content control that the user is leaving.|
| _Cancel_|必需|**Boolean**|Specifies whether to cancel the event.  **True** cancels the event and does not allow the user to leave the control.|

## Remarks


 **重要信息**  This event fires only for the content control that you exit and not for parent content controls. For example, if you have a text box content control nested inside a group content control, and you move the cursor from inside the text box content control and into another place in the document, this event fires only once for the text box content control; it does not fire for the parent group content control.

...
## Syntax
 ...
 _表达式_. **Add**( ** _Name_**, ** _Formula_**, ** _SolveOrder_**, ** _Type_** )
...
| _Formula_|必需|**String**|The formula of the calculated member.|
| _SolveOrder_|可选|**Variant**|The solve order for the calculated member.|
| _Type_|可选|**Variant**|The type of calculated member.|
| _Dynamic_|可选|**Boolean**|Specifies if the calculated member is recalculated with every update.|
| _DisplayFolder_|可选|**String**|The name of the display folder for the calculated member.|
| _HierarchizeDistinct_|可选|**Boolean**|Specifies whether to order and remove duplicates when displaying the hierarchy of the calculated member in a PivotTable report based on an OLAP cube.|
 ...
 **注释**  Connection to the cube and existing pivot table is necessary for the sample to run.

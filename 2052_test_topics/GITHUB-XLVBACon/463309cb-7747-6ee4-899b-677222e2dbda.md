...
## Syntax
 ...
 _表达式_. **FindB**( ** _Arg1_**, ** _Arg2_**, ** _Arg3_** )
...
| _Arg2_|必需|**String**|Within_text - the text containing the text you want to find.|
| _Arg3_|可选|**Variant**|Start_num - specifies the character at which to start the search. The first character in within_text is character number 1. If you omit start_num, it is assumed to be 1.|
 ...
 **重要信息**  FIND is intended for use with languages that use the single-byte character set (SBCS), whereas FINDB is intended for use with languages that use the double-byte character set (DBCS). The default language setting on your computer affects the return value in the following way:

...
## Syntax
 ...
 _表达式_. **AddCalculatedMember** _(Name,_ _Formula,_ _SolveOrder,_ _Type,_ _DisplayFolder,_ _MeasureGroup,_ _ParentHierarchy,_ _ParentMember,_ _NumberFormat)_
...
|:-----|:-----|:-----|:-----|
| _Name_|必需|STRING|The name of the calculated member.|
| _Formula_|必需|VARIANT|The formula of the calculated member.|
| _SolveOrder_|可选|VARIANT|The solve order for the calculated member.|
| _Type_|可选|VARIANT|The type of calculated member.|
| _DisplayFolder_|可选|VARIANT|A folder that exists to display calculated measures.|
| _MeasureGroup_|可选|VARIANT|The group to which the calculated belongs.|
| _ParentHierarchy_|可选|VARIANT|The parent path of the  **ParentMember**.|
| _ParentMember_|可选|VARIANT|The parent of the calculated member.|
| _NumberFormat_|可选|VARIANT|The format of numbers used for calculated members.|
 ...
 **注释**  In both of these samples, the PivotTable must be refreshed after creating the calculation in order to view it in the user interface.

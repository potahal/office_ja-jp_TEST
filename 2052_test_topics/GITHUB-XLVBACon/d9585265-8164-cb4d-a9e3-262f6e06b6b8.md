...
## Syntax
 ...
 _表达式_. **Find**( ** _What_**, ** _After_**, ** _LookIn_**, ** _LookAt_**, ** _SearchOrder_**, ** _SearchDirection_**, ** _MatchCase_**, ** _MatchByte_**, ** _SearchFormat_** )
...
|:-----|:-----|:-----|:-----|
| _What_|必需|**Variant**|The data to search for. Can be a string or any Microsoft Excel data type.|
| _After_|可选|**Variant**|The cell after which you want the search to begin. This corresponds to the position of the active cell when a search is done from the user interface. Notice that  _After_ must be a single cell in the range. Remember that the search begins after this cell; the specified cell isn?t searched until the method wraps back around to this cell. If you do no specify this argument, the search starts after the cell in the upper-left corner of the range.|
| _LookIn_|可选|**Variant**|The type of information.|
| _LookAt_|可选|**Variant**|Can be one of the following  **XlLookAt** constants: **xlWhole** or **xlPart**.|
| _SearchOrder_|可选|**Variant**|Can be one of the following  **XlSearchOrder** constants: **xlByRows** or **xlByColumns**.|
| _SearchDirection_|可选|[XlSearchDirection](fc53fda7-d01c-c407-ccdb-d12004a4dcee.md)|The search direction.|
| _MatchCase_|可选|**Variant**|**True** to make the search case sensitive. The default value is **False**.|
| _MatchByte_|可选|**Variant**|Used only if you have selected or installed double-byte language support.  **True** to have double-byte characters match only double-byte characters. **False** to have double-byte characters match their single-byte equivalents.|
| _SearchFormat_|可选|**Variant**|The search format.|

...
## Syntax
 ...
 _表达式_ A variable that returns a **Workbook** object.
...
|:-----|:-----|:-----|:-----|
| _SaveChanges_|可选|**Variant**|**True** to save the workbook to the server location. The default is **True**.|
| _Comments_|可选|**Variant**|Comments for the revision of the workbook being checked in (applies only if  _SaveChanges_ is set to **True** ).|
| _MakePublic_|可选|**Variant**|**True** to allow the user to publish the workbook after it is checked in.|
| _VersionType_|可选|**Variant**|Specifies versioning information for the workbook. |

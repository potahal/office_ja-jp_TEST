...
|:-----|:-----|:-----|:-----|
| _Value_|Obligatoire|**Variant**|A variable of data type **Variant**.|
| _ValueIfNull_|Facultatif|**Variant**|Optional (unless used in a query). A  **Variant** that supplies a value to be returned if the variant argument is **Null**. This argument enables you to return a value other than zero or a zero-length string.
 **Remarque**  If you use the  **Nz** function in an expression in a query without using the*valueifnull* argument, the results will be a zero-length string in the fields that contain null values.
